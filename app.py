...
def left_panel():
    ...
    st.sidebar.download_button("Download sample CSV", df.to_csv(index=False).encode("utf-8"), "sample_machine.csv", "text/csv")
    ...
    if uploaded is not None:
        try:
            ...
            if required.issubset(up.columns): df = up.copy()
            else: st.sidebar.error("Uploaded CSV missing required columns. Using bundled sample.")
    ...
    st.sidebar.markdown("---")
    diversity = st.sidebar.checkbox("Ensure variety in demo (diversity booster)", value=True,
...
def train_model(df):
    ...
    feats = ["age_months","ambient_temp_c","load_pct","run_hours","vibration_rms","pressure_bar","temperature_c","maintenance_overdue","year","month_num","month_idx","month_sin","month_cos","machine_id"]
    from collections import Counter
    c = Counter(y_enc); strat = y_enc if min(c.values())>=2 and len(c)>1 else None
    if strat is None: st.warning("Stratified split disabled due to rare classes.", icon="⚠️")
    X_train, X_test, y_train, y_test = train_test_split(X[feats], y_enc, test_size=0.25, random_state=42, stratify=strat)
    if _USE_CAT:
        w = _class_weights(y_train); class_weights=[w[i] for i in sorted(w.keys())]
        model = CatBoostClassifier(iterations=600, depth=6, learning_rate=0.08, loss_function="MultiClass",
        ...
        model.fit(X_train, y_train); y_pred_test = model.predict(X_test)
...
def generate_future(df, months_ahead=12, summer_boost=4.0):
    ...
    for m in machines:
        ...
        for i, d in enumerate(future_months, start=1):
            ...
            rows.append({"date":d.strftime("%Y-%m-%d"),"month":d.strftime("%b-%Y"),"machine_id":m,"age_months":age_months,"ambient_temp_c":round(ambient_temp_c,2),
                         "load_pct":round(load_pct,3),"run_hours":round(run_hours,1),"vibration_rms":round(vibration_rms,3),"pressure_bar":round(pressure_bar,2),
                         "temperature_c":round(temperature_c,2),"maintenance_overdue":maintenance_overdue})
...
def _basis_from_prediction(row, label):
    ...
    # Month number for seasonal hints (winter hydraulic)
    try:
    ...
    # Fallbacks to keep variety if we didn't hit thresholds
    if not reasons and label != "No Failure":
        # choose the top 2 generic contributors by simple scoring
        scores = [
...
def _format_top3(p, classes):
    order = np.argsort(p)[::-1][:3]; return ", ".join([f"{classes[i]} ({p[i]*100:.0f}%)" for i in order])
...
def _style_rows_with_confidence(df_to_style, defect_col="Defect", conf_col="Max Prob"):
    def to_style(row):
        ...
        if label != "No Failure":
            alpha = 0.15 + 0.55*min(1.0,max(0.15,conf)); return [f"background-color: rgba(255,82,82,{alpha}); font-weight: 600;" for _ in row]
        else:
            alpha = 0.10 + 0.4*min(1.0,max(0.1,conf)); return [f"background-color: rgba(46,204,113,{alpha}); font-weight: 500;" for _ in row]
    try: return df_to_style.style.apply(to_style, axis=1)
    except Exception: return df_to_style
...
# diversity booster: if one label dominates a month (>60%), flip 1-2 rows to their 2nd-best class
def diversify(pred_idx, proba, months, threshold=0.6):
    ...
    for m in uniq_months:
        ...
        if frac >= threshold and len(sel)>=3:
            # choose 2 lowest-confidence of the dominant label and switch to 2nd best
...
pred_df = Xf[["date","month","machine_id","temperature_c","vibration_rms","load_pct","pressure_bar","run_hours","maintenance_overdue"]].copy()
...
with st.expander("📊 Model validation — confusion matrix & metrics", expanded=False):
    ...
    fig = go.Figure(data=go.Heatmap(z=cm, x=labels_text, y=labels_text, colorscale="Blues", text=cm, texttemplate="%{text}", showscale=True))
...
if chosen_month != "All months":
    ...
    for m in sorted(pred_df["month"].unique(), key=lambda s: pd.to_datetime("01-"+s, format="%d-%b-%Y")):
        st.markdown(f"**{m}**")
...
# Preview with its own month control (defaults to follow the top selector)
st.markdown("### 📄 Preview source data (Jul 2024 → Jul 2025)")
mode = st.radio("Preview scope", ["Follow selection above", "All months", "Choose a month"], horizontal=True, index=0)
...
if mode == "Follow selection above" and chosen_month != "All months":
    ...
    months_src = sorted(preview["month"].unique().tolist(), key=lambda s: pd.to_datetime("01-"+s, format="%d-%b-%Y"))
